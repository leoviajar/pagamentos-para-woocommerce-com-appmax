...
namespace AppMax\WooCommerce\Gateway\Vendor\Piggly\ApiClient\Models;
...
class CredentialModel extends AbstractModel
{
    ...
    {
        return ['token_type' => $this->get('token_type', static::TOKEN_TYPE_BEARER), 'access_token' => $this->get('access_token'), 'scope' => $this->get('scope'), 'timezone' => $this->has('timezone') ? $this->get('timezone')->getName() : 'UTC', 'consented_on' => $this->has('consented_on') ? $this->get('consented_on')->getTimestamp() : null, 'expires_on' => $this->has('expires_on') ? $this->get('expires_on')->getTimestamp() : null, 'expires_in' => $this->get('expires_in')];
    }
    /**
