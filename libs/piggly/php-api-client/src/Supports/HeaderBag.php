...
namespace AppMax\WooCommerce\Gateway\Vendor\Piggly\ApiClient\Supports;
...
class HeaderBag
{
    ...
    public function append(string $key, $content)
    {
        ...
        $this->_headers[\strtolower($key)] = \array_merge($this->get($key), $this->parseContent($content));
        return $this;
    }
    /**
    ...
    private function parseRaw(string $raw)
    {
        ...
        foreach ($raw as $header) {
            ...
            $this->append(\trim($header[0]), \trim($header[1]));
        }
        return $this;
    }
    /**
    ...
    private function isJson($value)
    {
        ...
        $json_data = \json_decode($value, \true);
