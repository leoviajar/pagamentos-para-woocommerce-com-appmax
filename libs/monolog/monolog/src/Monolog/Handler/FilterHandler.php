...
use AppMax\WooCommerce\Gateway\Vendor\Monolog\ResettableInterface;
use AppMax\WooCommerce\Gateway\Vendor\Monolog\Formatter\FormatterInterface;
use AppMax\WooCommerce\Gateway\Vendor\Psr\Log\LogLevel;
/**
...
class FilterHandler extends Handler implements ProcessableHandlerInterface, ResettableInterface, FormattableHandlerInterface
{
    ...
    public function __construct($handler, $minLevelOrList = Logger::DEBUG, $maxLevel = Logger::EMERGENCY, bool $bubble = \true)
    {
        ...
        if (!$this->handler instanceof HandlerInterface && !\is_callable($this->handler)) {
            throw new \RuntimeException("The given handler (" . \json_encode($this->handler) . ") is not a callable nor a AppMax\\WooCommerce\\Gateway\\Vendor\\Monolog\\Handler\\HandlerInterface object");
        }
    }
    /**
    ...
    public function setAcceptedLevels($minLevelOrList = Logger::DEBUG, $maxLevel = Logger::EMERGENCY) : self
    {
        if (\is_array($minLevelOrList)) {
            $acceptedLevels = \array_map('AppMax\\WooCommerce\\Gateway\\Vendor\\Monolog\\Logger::toMonologLevel', $minLevelOrList);
        } else {
