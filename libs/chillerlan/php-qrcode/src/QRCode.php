 ...
 * @license      MIT
 */
namespace AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode;

use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Data\MaskPatternTester;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Data\QRCodeDataException;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Data\QRDataInterface;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Data\QRMatrix;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRCodeOutputException;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRFpdf;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRImage;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRImagick;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRMarkup;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QROutputInterface;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\QRCode\Output\QRString;
use AppMax\WooCommerce\Gateway\Vendor\chillerlan\Settings\SettingsContainerInterface;
...
class QRCode
{
    ...
    public const OUTPUT_MODES = [QRMarkup::class => [self::OUTPUT_MARKUP_SVG, self::OUTPUT_MARKUP_HTML], QRImage::class => [self::OUTPUT_IMAGE_PNG, self::OUTPUT_IMAGE_GIF, self::OUTPUT_IMAGE_JPG], QRString::class => [self::OUTPUT_STRING_JSON, self::OUTPUT_STRING_TEXT], QRImagick::class => [self::OUTPUT_IMAGICK], QRFpdf::class => [self::OUTPUT_FPDF]];
    /**
    ...
    public function getMatrix(string $data) : QRMatrix
    {
        ...
        $maskPattern = $this->options->maskPattern === $this::MASK_PATTERN_AUTO ? $this->getBestMaskPattern() : $this->options->maskPattern;
    ...
    protected function getBestMaskPattern() : int
    {
        ...
        for ($pattern = 0; $pattern < 8; $pattern++) {
            $tester = new MaskPatternTester($this->dataInterface->initMatrix($pattern, \true));
    ...
    public function initDataInterface(string $data) : QRDataInterface
    {
        ...
        if (in_array($this->options->dataMode, $dataModes, \true)) {
    ...
    protected function initOutputInterface(string $data) : QROutputInterface
    {
        ...
        foreach ($this::OUTPUT_MODES as $outputInterface => $modes) {
            if (in_array($this->options->outputType, $modes, \true) && class_exists($outputInterface)) {
    ...
    protected function checkString(string $string, array $charmap) : bool
    {
        $len = strlen($string);
        for ($i = 0; $i < $len; $i++) {
            if (!in_array($string[$i], $charmap, \true)) {
                return \false;
            }
        }
        return \true;
