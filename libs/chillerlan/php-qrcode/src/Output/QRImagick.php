...
class QRImagick extends QROutputAbstract
{
    ...
    protected function setModuleValues() : void
    {
        foreach ($this::DEFAULT_MODULE_VALUES as $type => $defaultValue) {
            ...
            if (!is_string($v)) {
                $this->moduleValues[$type] = $defaultValue ? new ImagickPixel($this->options->markupDark) : new ImagickPixel($this->options->markupLight);
            } else {
    ...
    protected function drawImage() : void
    {
        ...
        foreach ($this->matrix->matrix() as $y => $row) {
            foreach ($row as $x => $M_TYPE) {
                ...
                $draw->rectangle($x * $this->scale, $y * $this->scale, ($x + 1) * $this->scale, ($y + 1) * $this->scale);
            }
        }
